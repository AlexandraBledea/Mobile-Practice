  ...
  showAlertDialogForNotification(BuildContext context, Entity item) {
    ...
    AlertDialog alert = AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text("Description: ${item.type}"),
        const SizedBox(
          height: 10.0,
        ),
        Text("Category: ${item.distance}"),
        const SizedBox(
          height: 10.0,
        ),
        Text("Image: ${item.duration}"),
        const SizedBox(
          height: 10.0,
        ),
        Text("Units: ${item.calories}"),
        const SizedBox(
          height: 10.0,
        ),
        Text("Price: ${item.rate}"),
        const SizedBox(
          height: 10.0,
        )
      ]),
                ...
                children: <Widget>[
                    ...
                    if (storage.connected.value == ConnectionStatus.CONNECTED) {
                      if (value == 0) {
                        Navigator.push(context,
                          ...
                          return const ProgressScreenWidget(); //TODO PROGRESS SCREEN
                        }));
                      }
                    } else {
                  ...
                  Expanded(
                      child: ListView.builder(
                          ...
                          itemCount: storage.dates.length,
                          itemBuilder: (BuildContext context, int index) {
                            var item = storage.dates[index];
                            return InkWell(
                                child: Card(
                                    color: Colors.white70,
                                    child: ListTile(
                                      title: Column(children: [
                                        Text(storage.dates[index])
                                      ]),
                                      onTap: () {
                                        print("should change");
                                        currentlySelectedCategory = item;
                                        storage.getEntries(item);
                                      },
                                    )));
                          })),
                  Expanded(
                      child: ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            return Card(
                                ...
                                child: ListTile(
                                  title: Column(children: [
                                    Text("Name: ${storage.entries[index].date}"),
                                    Text("Description: ${storage.entries[index].type}"),
                                    Text(
                                        "Image: ${storage.entries[index].duration}"),
                                    Text("Category: ${storage.entries[index].distance}"),
                                    Text("Units: ${storage.entries[index].calories}"),
                                    Text("Price: ${storage.entries[index].rate}")
                                  ]),
                                  trailing: SizedBox(
                                    width: 70,
                                    child: Row(
                          ...
                          return Card(
                              ...
                              child: ListTile(
                                title: Column(children: [
                                  Text("Name: ${storage.entries[index].date}"),
                                  Text("Description: ${storage.entries[index].type}"),
                                  Text("Image: ${storage.entries[index].duration}"),
                                  Text("Category: ${storage.entries[index].distance}"),
                                  Text("Units: ${storage.entries[index].calories}"),
                                  Text("Price: ${storage.entries[index].rate}")
                                ]),
